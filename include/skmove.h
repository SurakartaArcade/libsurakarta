...
inline int getMoveStartRow(Move mv)
{
  return mv & ((1 << 6) - 1);
}

inline int getMoveStartColumn(Move mv)
{
  return (mv >> 6) & ((1 << 6) - 1);
}

inline int getMoveFinalRow(Move mv)
{
  return (mv >> 12) & ((1 << 6) - 1);
}

inline int getMoveFinalColumn(Move mv)
{
  return (mv >> 18) & ((1 << 6) - 1);
}

inline bool isMoveAttack(Move mv)
{
  return (mv >> 19) & 1;
}

inline Bid getMoveBid(Move mv)
{
  return (Bid)((mv >> 20) & ((1 << 3) - 1));
}
